...
17. **Project Halo, Jack's name for v2**: pooled compute lets multiple members' machines load a single larger agent "blob" together than any one of them could alone — the plain-language framing for what D15/D16 above describe more technically. Research phase starts by identifying the real technological limitations and concepts involved (network bandwidth/latency between member machines vs. exo's assumed-LAN sharding model, heterogeneous hardware/quantization mismatches across arbitrary volunteer machines, checkpoint/failure semantics when a shard-holding node drops mid-inference, and how this interacts with ADR-006's sandbox trust boundary once inference itself is distributed across machines that don't trust each other the way LAN-clustered hardware does) — not yet scoped as an ADR; first review is the 2026-09-10 10am session.
    ...
    **Project Halo now lives in its own repo: [jackcanon/halo](https://github.com/jackcanon/halo)** (private, created 2026-09-10; local checkout `Apps/Halo-src`). HaloBench, all reports, the test log, the v2 integration lessons, and the volunteer kit moved there; the copies under this repo's `apps/halo-bench` and `docs/HALO-*`/`docs/halo-*` are the pre-move originals and should be deleted rather than edited. The Halo ADR, when written, goes in that repo's `ADR/`; this decision keeps the cross-reference.

    **Amended 2026-09-10 — first hands-on results.** The LAN capability tier is demonstrated: Llama 3.3 70B Q4_K_M (42.5 GB) ran pooled across four lab machines (M4 Max host + M4 Pro + RTX 4070 + M1 Pro, llama.cpp RPC) at 51 pp512 / 5.0 tg128, reproduced within 0.5%, and at 68 / 6.7 three-way. No lab machine holds that model alone. The substrate is llama.cpp's RPC backend, not exo (D16's candidate) — exo is Apple-only, LAN/Thunderbolt-only, and not needed for this tier. Eight integration lessons for the v2 sharded runner, scheduler and capability model are recorded in `docs/HALO-V2-INTEGRATION-LESSONS.md` (L1 host loads with mmap off; L2 smallest-pool-first placement with a validated per-node cost model; L3 measured, not nominal, capability records; L4 worker launch contract; L5 diagnostics; L6 R7 shard-death gates `shard_plan`, which also does not yet exist as a live column; L7 LAN speed curve; L8 one quant per pooled job) and tracked in Cmd Work. WAN, trust boundary, and failure semantics remain open; Test 02 (RTT curve) and R7/R8 are the next gates. The Halo ADR follows from those.
