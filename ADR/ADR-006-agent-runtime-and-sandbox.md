...
1. **Agent loop lives in the Rust node core (D41, D43).** The `ohhive` core crate ships an `agent` module: a deterministic step machine `Plan → Act(tool|generate) → Observe → Review → Done`, driven by the node's `Backend` trait (ADR-003). The hub has no agent loop for cards; hub-side code is limited to the interviewer/planner (decision 8) and scheduling.
