...
Their Queen Bee has direct read/write access to a shared credential store across all of one operator's own trusted agents. That model assumes single-operator trust throughout. Hive's sandbox (ADR-006) exists specifically because a card's executing node cannot be assumed trustworthy -- importing shared-credential access across nodes would undo the reason that sandbox exists. Nothing in this ADR proposes adopting that part of their architecture.
