...
- Where does distributed-inference (v2) shard coordination live — in `ohhive-core` or a separate crate wrapping exo? Open; only the schema slot is reserved now.
...
- ADR-004-p2p-overlay-and-regional-servers.md — libp2p in `ohhive-core`; regional servers as model cache.
