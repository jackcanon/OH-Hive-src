...
## Follow-up: explicit compute reapproval — 2026-09-16

Migration `20260916050800_compute_budget_reapproval.sql` adds owner-only replacement of a community project compute budget. A card must be suggested/ready/blocked with no lease or reservation. Interview-wallet approvals are excluded. The request includes the previously displayed approval timestamp and current input/requirements hash; stale submissions fail and require refresh. Replacement locks the card and project, archives the previous approval in a member-readable history table, then freezes current rates and requirements. The entire operation rolls back if validation fails. Prior ledger charges remain; the new limit is a lifetime total that must exceed past spending, not an additional allowance. Replacement does not fund a project or requeue a job. Current claim paths acquire the card lock before lease insertion.

`ComputeBudget.tsx` exposes the explicit replacement action when the status RPC allows it, with copy stating that prior spending counts and no refund occurs. Currently mounted text-card scope remains unchanged; the endpoint also accepts the existing core-supported code modality. S-4’s replacement-approval gap is closed for these project-board callers, not for every client or interview flow.

Verification: all 98 migrations replay in PGlite; tests cover owner/member denial, private-card rejection, stale approval/input rejection, lifetime-spend retention, changed-input reapproval, active-lease refusal, history creation and rollback of history/deletion on invalid precision. Web TypeScript, migration/RLS guard, whitespace checks and baseline comparison pass (1,070 catalog objects, 326 functions, same 22 documented pending changes). PGlite is not proof of real concurrent PostgreSQL behavior; claim-versus-reapproval contention and browser acceptance remain rollout checks. No deployment, commit, push or actual spending performed.

Remaining S-4: worker-side execution caps, infrastructure storage administration and production acceptance/deployment. S-5 held-chain controls remain queued. Sif your friendly Codex Agent
