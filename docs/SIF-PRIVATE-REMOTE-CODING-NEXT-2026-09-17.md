...
## Implemented: expiring host readiness and native bridge

Schema v21 adds private_coding_readiness. A verified node may publish only its own worker/coding opt-in, Git sign-in indicator and up to 64 unique model IDs with confirmed/unsupported/unknown tool metadata. Caller-supplied identity/timestamps are not accepted. The authority stamps receipt time; reports expire after 45 seconds, including rejection of future timestamps as fresh. Listings include enrolled same-owner nonrevoked nodes even when no report exists, distinguishing enrollment from availability.

`refresh_private_coding_readiness` queries model metadata only. Tool probes run with concurrency eight and an eight-second overall tool-discovery deadline. Unavailable model servers clear earlier model availability; tool metadata failures remain explicitly unconfirmed. Reports are host assertions, not proof of per-repository Git access or permission to execute. Always check freshness before offering execution; fresh but disabled is still disabled. Live execution retains all preflight gates.

`HiveNode::private_coding_hosts` and typed `PrivateCodingHost` bridge this data to the primary desktop app. The Swift picker, periodic target publication and worker invocation are still not wired; this change does not advertise the real Overgaard as ready. FFI compile/strict Clippy passed; full LocalHub suite 111 passed, plus targeted mock-server rerun after bounded discovery change. No model inference/native rebuild/deployment. Logs `/private/tmp/hive-readiness-{suite,refresh,ffi,clippy}.log`.
