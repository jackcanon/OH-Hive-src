...
## Shared GitHub App device flow implemented — 2026-09-16

Supersedes the earlier browser/BYOK implementation and pending flow decision. ADR-036 decision 1 and Claude review `75ef7f0` direct a publisher-owned GitHub App with device flow. GitHubAuthManager now requests a device code, shows the code and fixed GitHub verification link, polls at the provider interval, honors pending/slow-down responses, handles denial/expiry, and supports cancellation. Cancellation/disconnect invalidates the attempt so late responses cannot restore credentials or UI state. No desktop client secret or loopback listener remains. Refresh uses the secretless exception for device-issued tokens. Access and rotated refresh tokens are saved together in one checked Keychain item bound to the configured client ID; legacy/unmatched sessions cannot authenticate.

Build configuration: set the public `HIVE_GITHUB_OAUTH_CLIENT_ID` when running `apps/desktop-swift/scripts/build-app.sh`. It embeds `HiveGitHubOAuthClientID` in Info.plist before signing. Without this configuration the Connect control is unavailable. Do not supply an app secret/private key. Enable Device Flow on the publisher GitHub App, review its actual permissions, and install it where required. GitHub App permissions come from registration, not OAuth scopes. This screen still only lists up to 100 public repositories; installation-aware private repository selection, git credentials/writes and Copilot consumer wiring remain separate work.

Jack reports the GitHub App has now been created. At this check the local log/queue did not yet contain its public client ID or device-flow enablement. Requested those details; registration is no longer assumed absent. Live device approval, refresh, account permissions, cancellation UI and packaged-app acceptance remain unverified.

Validation: all 21 Swift tests pass, including seven GitHub tests covering pending/slow-down intervals, expiration before polling, denial, cancellation, client binding/legacy rejection, token decoding, safe links and credential readback. Shell syntax and whitespace checks pass. Tests use fake time/responses, never real credentials. No commit, push, release bundle, production deployment or live sign-in performed.

Sources: https://docs.github.com/en/apps/creating-github-apps/authenticating-with-a-github-app/generating-a-user-access-token-for-a-github-app and https://docs.github.com/en/apps/creating-github-apps/authenticating-with-a-github-app/refreshing-user-access-tokens .
