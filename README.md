...
Pooled compute for the community. Members contribute idle computers to **the Hive**, earn **$honey**, and spend it on agent-driven projects — text, code, image, video, audio — run from a community kanban.
...
```
crates/hive-core          shared core: capabilities, jobs, Backend trait, node record  (ADR-003)
...
apps/desktop              Tauri "Hive" node app; wraps the core, adds backends + Python sidecar (ADR-010)
...
```sh
# Rust (all OSes)
cargo test --workspace --exclude hive-desktop
...
pnpm --filter @hive/desktop tauri dev                 # needs Rust + Tauri prerequisites
```
...
- Everything Hive lives in Postgres schema `hive`. Never touch `public`.
...
- Work is tracked in Cmd Work (project: Hive), not in ad-hoc kanbans.
