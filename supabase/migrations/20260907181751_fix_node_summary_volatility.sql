-- hive.node_summary (and its public wrapper) were declared STABLE, but they call
-- hive.verify_node_key(), which does a real UPDATE (touches node_keys.last_used_at). PostgREST
-- runs STABLE-declared RPCs in a read-only transaction, so every single call to node_summary was
-- failing with "cannot execute UPDATE in a read-only transaction" (surfaced to the desktop app,
-- which polls this every 15s) — the nested write inside a function this one calls was never
-- something node_summary's own STABLE label was entitled to claim. Every other function in this
-- schema that touches verify_node_key is (correctly) volatile; node_summary was the one exception.
-- Body unchanged, volatility only.
