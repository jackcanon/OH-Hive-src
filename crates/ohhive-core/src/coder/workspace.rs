...
use crate::canonical_path::canonical;
...
mod tests {
    ...
    fn fixture() -> (PathBuf, CodeSessionSpec) {
        ...
        git(&source, &["init"]);
        // Git for Windows installs set core.autocrlf=true globally, and that applies to the
        // clone the product makes, not only to this source repo -- so a committed "base\n" came
        // back as "base\r\n" and failed an equality assertion that had nothing to do with line
        // endings. `.gitattributes` travels with the repository through clone and worktree, which
        // is the only form of this fix that reaches where the content is actually read.
        std::fs::write(source.join(".gitattributes"), "* -text\n").unwrap();
        std::fs::write(source.join("tracked.txt"), "base\n").unwrap();
        git(&source, &["add", ".gitattributes", "tracked.txt"]);
