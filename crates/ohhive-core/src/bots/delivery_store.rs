...
pub trait DeliveryStore: BotsService + Send + Sync {
    async fn agent_bio(&self, _owner: UserId, _agent: AgentId) -> BotsResult<AgentBio> {
        Ok(AgentBio::default())
    }
    async fn user_profile(&self, _owner: UserId) -> BotsResult<UserProfile> {
...
impl DeliveryStore for LocalHubStore {
    async fn agent_bio(&self, owner: UserId, agent: AgentId) -> BotsResult<AgentBio> {
        self.bots_agent_bio_get(owner, agent).map_err(Into::into)
    }
    async fn user_profile(&self, owner: UserId) -> BotsResult<UserProfile> {
        self.bots_user_profile_get(owner).map_err(Into::into)
    }
    async fn message_get(&self, id: MessageId) -> BotsResult<Message> {
...
impl DeliveryStore for RemoteLocalHub {
    async fn agent_bio(&self, _owner: UserId, agent: AgentId) -> BotsResult<AgentBio> {
        self.bots_agent_bio_get(agent).await.map_err(Into::into)
    }
    async fn user_profile(&self, _owner: UserId) -> BotsResult<UserProfile> {
        self.bots_user_profile_get().await.map_err(Into::into)
    }
    async fn message_get(&self, id: MessageId) -> BotsResult<Message> {
