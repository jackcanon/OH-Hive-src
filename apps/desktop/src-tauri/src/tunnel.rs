//! Cloudflare Tunnel automation (ADR-013 D74, gateway option a). Bundled `cloudflared` gives a
//! regional server a public HTTPS hostname with no manual CLI, no port forwarding, no
//! certificates -- matching the "Setup does the rest" bar the Ollama installer already holds.
//!
//! Flow: `login` opens the printed URL and waits for `~/.cloudflared/cert.pem` to appear, then
//! `create` + `route_dns` + `write_config` produce a config the bundled binary can `run` in the
//! background for as long as the regional server role is on (lib.rs ties its lifecycle to
...
pub fn bundled_path(app: &tauri::AppHandle) -> Option<PathBuf> {
    use tauri::Manager;
    let rel = format!(
...
/// typically shelling to `open` like setup.rs does for Ollama), and wait up to 5 minutes for
/// `cert.pem` to land. Cloudflare's own page confirms success; we only watch the filesystem.
